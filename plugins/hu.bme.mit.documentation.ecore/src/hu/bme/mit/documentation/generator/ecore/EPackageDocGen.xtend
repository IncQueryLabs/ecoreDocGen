 ...
 * 
 */
class EPackageDocGen {
	private EPackage pckg
    private StringBuilder builder
    private List<String> filter
    
    def documentEPackageToLatex(StringBuilder sb, EPackage pckg, List<String> nameRefFilter){
        this.builder = sb
        this.pckg = pckg
        this.filter = Lists::newArrayList(nameRefFilter)
        this.filter.add(pckg.nsURI)
        ...
        pckg.EClassifiers.sortBy[name].forEach[
        	...
        	if(it instanceof EClass){
        		...
        		if(!cls.EAttributes.empty){
	        		...
	        		'''
					\end{tabularx}
					\caption{Attributes of the «cls.name» EClass}
					\label{«cls.EPackage.nsPrefix+"."+cls.name+".attr"»}
					\end{table}
	        		'''.appendToBuilder
        		...
        		if(!cls.EReferences.empty){
	        		...
	        		'''
					\end{tabularx}
					\caption{References of the «cls.name» EClass}
					\label{«cls.EPackage.nsPrefix+"."+cls.name+".ref"»}
					\end{table}
	        		'''.appendToBuilder
        		...
        		if(!cls.EOperations.empty){
	        		...
	        		'''
	        		\end{tabularx}
	        		\caption{Operations of the «cls.name» EClass}
	        		\label{«cls.EPackage.nsPrefix+"."+cls.name+".op"»}
	        		\end{table}
	        		'''.appendToBuilder
    ...
    def private documentEPackageHeader(EPackage pckg)
		...
		«documentHeader("section", title, pckg.name, pckg.nsPrefix, pckg)»
		\paragraph{EPackage properties:} \hspace{0pt} \\ \indent
		«documentProperty("Namespace Prefix", '''«pckg.nsPrefix»''')»
	...
	\texttt{«literal.literal»} & «literal.value» &
	«literal.findGenModelDocumentation(false)» \\ \hline
    «ENDFOR»
	\end{tabularx}
	\caption{Literals of the «enum.name» EEnum}
	\label{«enum.EPackage.nsPrefix+"."+enum.name+".lit"»}
	\end{table}
    '''
    ...
    '''
    \texttt{«IF color != null»\color{«color»}{«ENDIF»«elem.name»«IF color != null»}«ENDIF»}
    '''
    ...
    def private preparePossibleReference(EClassifier cls){
    	...
    	if(filter.findFirst[typePckg.nsURI.contains(it)] != null){
    		'''\nameref{«typePckg.nsPrefix».«typeName»}'''
    	} else {
    		typeName
    	}
    }
    ...
    \newline
    «documentProperty("Default", feat.defaultValueLiteral)»
    «ENDIF»
    «IF feat.derived»
    ...
    «FOR param : op.EParameters»
    	\item «param.preparePossibleReference»[«param.lowerBound»..«IF param.upperBound == ETypedElement::UNBOUNDED_MULTIPLICITY»*«ELSE»«param.upperBound»«ENDIF»] \texttt{«param.name»}
    «ENDFOR»
    \end{itemize}
    «ENDIF»
    '''
    ...
    '''
    \«sectionClass»[«shortTitle»]{«sectionTitle»}
    \label{«label»}
    ...
    def private findGenModelDocumentation(EModelElement element, boolean required){
    	...
    	if(doc != null){
    		var itemizeChar = "* "
    		val itemizeCharDefaultLength = itemizeChar.length
    		val sb = new StringBuilder
    		val lineFeedNormalDoc = doc.replace("\r\n","\n")
    		for(s : lineFeedNormalDoc.split("\n")){
    			if(s.startsWith(" "+itemizeChar)){
    				sb.append('''\begin{itemize}'''+"\n")
    				itemizeChar = " "+itemizeChar 
    			} else {
	    			val index = s.indexOf('*')
	    			if(index > 0 && index < itemizeChar.length-itemizeCharDefaultLength){
	    				val shortItemize = itemizeChar.substring(0,index).concat("* ")
	    				if(s.startsWith(shortItemize)){
		    				sb.itemizeEnd(itemizeChar.length-(shortItemize.length+1))
		    				itemizeChar = shortItemize;
	    				}			
	    			}
    			}
    			if(s.startsWith(itemizeChar)){
    				sb.append('''\item «s.substring(itemizeChar.length)»'''+"\n")
    			} else {
    				sb.append(s+"\n")
    			}
    			if("".equals(s) && itemizeChar.length > itemizeCharDefaultLength){
    				sb.itemizeEnd(itemizeChar.length-(itemizeCharDefaultLength+1))
    				itemizeChar = "* "
    			}
    		}
    		if(itemizeChar.length > itemizeCharDefaultLength){
    			sb.itemizeEnd(itemizeChar.length-(itemizeCharDefaultLength+1))
    		}
    		val refDoc = sb.toString.replaceAll('''\#\{(.*?)\}''','''\\nameref{$1}''')
    		val urlDoc = refDoc.replaceAll('''\@\{(.*?)\}''','''\\url{$1}''')
    		val pDoc = urlDoc.replaceAll('''b\{(.*?)\}''','''\\textbf{$1}''')
    		return pDoc
    	} else {
    		if(required){
    ...
    def private itemizeEnd(StringBuilder sb, int level){
    	(0..level).forEach[
			sb.append('''
			\end{itemize}
			''')
